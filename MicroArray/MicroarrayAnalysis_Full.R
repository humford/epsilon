...
cancer.names <- c("Glioblastoma", "OV", "LumA", "AML", "AML NK")
...
names(ControlSkew) <- rownames(controlMatrix)
...
pdf("Microarray Control_Skews.pdf")
...
pdf("Microarray_Control_Sampling.pdf")

plot(reject, lower[reject],col = "red", main= "Microarray Control Gene Spliting by Sample Size", xlab = "Samples", ylab = "Fraction of Genes in Lower Mode", 
...
for(cancer in cancer.names)
{
  ...
  pdf(paste(cancer, "_Skews.eps", sep = ""))
...
setwd("~/Documents/Graphs")
write.table(convergenceTable, "Microarray_Convergence")
...
pdf("Microarray_Splitting_Comparison.eps")
par(mai = c(1,2,1,1))
